...
`RHD-DIP-001A`–`RHD-DIP-001E` cover all 86 accepted starts of `RHD-FR-001`. `RHD-DIP-002A`–`RHD-DIP-002B` cover all 85 accepted starts of `RHD-FR-002`. `RHD-DIP-003A`–`RHD-DIP-003E` cover all 89 accepted starts of `RHD-FR-003`. `RHD-DIP-004A`–`RHD-DIP-004E` cover all 90 accepted starts of `RHD-FR-004`, on printed pages 357–361.

The cumulative layer currently contains **350 complete AI-assisted diplomatic article transcriptions**, exactly matching the 350 accepted article starts among the first 400 facsimile-reviewed high-confidence candidates. Ambiguous diacritics, compact letter sequences, unusual printer marks and inferential cross-references are retained or flagged in `uncertainty_note` rather than silently normalized.
