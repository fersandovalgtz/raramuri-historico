...
def pattern_candidates(keys,seed_keys,kind):
    ...
    # Remove redundant shorter patterns when a longer pattern has exactly the same membership.
    keep=[]
...
def main():
    ...
    for r in rows():
        ...
        for c in split_components(r.get('headword_diplomatic','')):
            k=norm(c)
            if alen(k)>=3:
                rar.append({'record_id':r.get('record_id',''),'form_diplomatic':c,'graphic_key':k,'printed_page':int(r.get('printed_page') or 0)})
    rar_by_key=defaultdict(list)
    for x in rar:rar_by_key[x['graphic_key']].append(x)
    recovery_by_key={x['graphic_key']:x for x in recovery}
    seed_keys={x['graphic_key'] for x in seeds}
    universe=sorted(set(rar_by_key)|set(recovery_by_key))

    prefixes=pattern_candidates(universe,seed_keys,'prefix')
    suffixes=pattern_candidates(universe,seed_keys,'suffix')
    patterns=prefixes+suffixes
    for i,x in enumerate(patterns,1):
        x['pattern_id']=f'RHD-GPAT-{i:05d}'
        x['status']='machine_graphic_pattern_hypothesis'
        x['human_reviewed']=False
        x['interpretive_scope']='Recurrent string pattern only; not a morpheme, morphological rule, phonological correspondence or validated linguistic unit.'
    ...
    for seed in sorted(seeds,key=lambda x:x['graphic_key']):
        ...
        for k in universe:
            ...
            long_shared=max(cp,cs)>=max(4,int(round(mn*.55)))
            bounded_edit=(d<=2 and mx<=10) or (d<=3 and mx>10 and s>=.75)
            ...
            source='rar_de_inventory' if k in rar_by_key else 'residual_recovery'
            payload={'graphic_key':k,'source_layer':source,'edit_distance':d if d<=4 else None,'similarity':s,'shared_prefix_length':cp,'shared_suffix_length':cs,'evidence_reasons':reasons}
            if source=='rar_de_inventory':
                payload['rar_de_attestations']=rar_by_key[k]
            else:
    ...
    suffix_focus=[x for x in patterns if x['pattern_type']=='recurrent_suffix_candidate'][:30]
    prefix_focus=[x for x in patterns if x['pattern_type']=='recurrent_prefix_candidate'][:30]
    summary={'dataset':'raramuri-historico-steffel-1809','generated':'2026-08-13','grade_a_seed_count':len(seeds),'strong_profile_recovery_universe_count':len(recovery),'rar_de_graphic_key_count':len(rar_by_key),'family_hypothesis_count':len(neighborhoods),'families_with_neighbors':sum(x['neighbor_count']>0 for x in neighborhoods),'total_neighbor_links':sum(x['neighbor_count'] for x in neighborhoods),'recurrent_prefix_pattern_count':len(prefixes),'recurrent_suffix_pattern_count':len(suffixes),'top_prefix_patterns':[{'pattern':x['pattern'],'member_count':x['member_count'],'grade_a_seed_count':x['grade_a_seed_count']} for x in prefix_focus],'top_suffix_patterns':[{'pattern':x['pattern'],'member_count':x['member_count'],'grade_a_seed_count':x['grade_a_seed_count']} for x in suffix_focus],'human_reviewed':False,'automatic_morphological_analysis':False,'automatic_morpheme_assignment':False,'automatic_lexeme_creation':False,'method':'grade_a_seeded_graphic_family_hypotheses_v1'}
    dump(OUT/'graphic_family_hypotheses.json',{'dataset':summary['dataset'],'layer':'steffel_graphic_family_hypotheses','generated':summary['generated'],'count':len(neighborhoods),'human_reviewed':False,'automatic_morphological_analysis':False,'automatic_morpheme_assignment':False,'records':neighborhoods})
    ...
    with (OUT/'graphic_family_hypotheses.csv').open('w',encoding='utf-8',newline='') as f:
        ...
        for x in neighborhoods:
            w.writerow({'family_hypothesis_id':x['family_hypothesis_id'],'seed_recovery_id':x['seed_recovery_id'],'seed_graphic_key':x['seed_graphic_key'],'seed_surface_forms':' | '.join(x['seed_surface_forms']),'seed_de_rar_headwords':' | '.join(x['seed_de_rar_headwords']),'seed_occurrence_count':x['seed_occurrence_count'],'neighbor_count':x['neighbor_count'],'neighbor_graphic_keys':' | '.join(n['graphic_key'] for n in x['neighbors']),'human_reviewed':False})
