...
def main():
    ...
    for r in rows():
        if active(r) and r.get('direction')=='RAR-DE':
            rar.update(norm(x) for x in split_components(r.get('headword_diplomatic','')))
    out=[]
    for x in src:
        c=corr.get(x['alignment_id'])
        if c: label=c['local_german_label'];method='facsimile_correction';surface=c['facsimile_reading']
        else:
            label='';method=''
        ...
        out.append({'alignment_id':x['alignment_id'],'member_id':x['member_id'],'record_id':x['record_id'],'printed_page':x['printed_page'],'article_headword':x['article_headword'],'analysis_surface':surface,'proposed_local_german_label':label,'proposal_method':method,'human_reviewed':False})
    s={'dataset':'raramuri-historico-steffel-1809','generated':'2026-08-13','count':len(out),'facsimile_correction_count':sum(x['proposal_method']=='facsimile_correction' for x in out),'fallback_count':sum(x['proposal_method']=='article_headword_fallback' for x in out),'human_reviewed_count':0}
