#!/usr/bin/env python3
"""Generate a conservative first cohort of Steffel ↔ Rarámuri Digital candidates.

This script creates *machine candidates*, never linguistic equivalences. It compares
Rarámuri headwords from the Steffel RAR-DE section with contemporary Rarámuri
headword components from the pinned Rarámuri Digital snapshot. Only exact matches
under a deliberately narrow graphic normalization are emitted.
"""
...
def normalized_graphic_key(value: str) -> str:
    """Conservative comparison key; not a phonological or linguistic normalization."""
    value = (value or "").strip().translate(APOSTROPHES).translate(DASHES)
    value = value.replace("ſ", "s").replace("ß", "ss")
    value = unicodedata.normalize("NFKD", value)
    value = "".join(ch for ch in value if unicodedata.category(ch) != "Mn")
    value = value.casefold()
    # Preserve spaces, apostrophes and hyphens because deleting them would create
    # a stronger equivalence claim than this first documentary pass warrants.
...
def split_components(value: str, historical: bool = False) -> list[str]:
    """Split only explicit variant separators; do not infer morphology."""
...
def main() -> None:
    ...
    for row in historical_rows:
        ...
        for historical_component_index, historical_component in enumerate(
            ...
            for modern_match in matches:
                m = modern_match["row"]
                candidates.append({
                    "correspondence_id": "",  # assigned after deterministic sorting
                    "historical": {
                        "record_id": rid,
                    ...
                    "candidate_relation": {
                        ...
                        "interpretive_scope": (
                            ...
                            "does not assert semantic identity, cognacy, lexical continuity, dialect identity or normative equivalence."
                        ),
                    },
                    "derivation": {
    ...
    summary = {
        ...
        "method": "conservative_exact_graphic_match_v1",
        "scope_note": (
            "Counts describe documentary comparison candidates only. No candidate is a validated "
            "semantic, etymological, dialectal or historical-continuity relation."
        ),
    }
    ...
    with OUT_CSV.open("w", encoding="utf-8", newline="") as f:
        ...
        for x in candidates:
            w.writerow({
                ...
                "short_form_warning": str(x["candidate_relation"]["short_form_warning"]).lower(),
    ...
    print(
        ...
        f"{len(modern_records_matched)} modern records; {len(matched_keys)} keys."
    )
