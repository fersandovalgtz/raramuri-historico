...
FILES = [
    ...
    ("second_source_minimal_canonical", "data/pilot/tellechea-1826.minimal-pilot.jsonl"),
    ("second_source_minimal_tei", "data/pilot/tellechea-1826.minimal-pilot.tei.xml"),
    ("second_source_minimal_diagnostics", "data/pilot/tellechea-1826.minimal-pilot.diagnostics.json"),
    ("second_source_full_canonical", "data/pilot/tellechea-1826.full-witness.jsonl"),
    ("second_source_full_tei", "data/pilot/tellechea-1826.full-witness.tei.xml"),
    ("second_source_full_diagnostics", "data/pilot/tellechea-1826.full-witness.diagnostics.json"),
...
def main():
    ...
    minimal_diag=read_json(ROOT/"data/pilot/tellechea-1826.minimal-pilot.diagnostics.json")
    full_diag=read_json(ROOT/"data/pilot/tellechea-1826.full-witness.diagnostics.json")
    minimal_count=jsonl_count(ROOT/"data/pilot/tellechea-1826.minimal-pilot.jsonl")
    ...
    credit=pilot_profile.get("completion_credit",{})
    fixed=(pilot_profile.get("witness",{}).get("identity_status")=="checksum_fixed_public_witness" and pilot_witness.get("status")=="checksum_fixed_public_witness")
    ...
    full_complete=(
        full_count==205
        and full_diag.get("pilot_id")=="RHD-TELLECHEA-1826-FULL-WITNESS-PILOT-1"
        and full_diag.get("pdf_pages")==205
        and full_diag.get("canonical_records")==205
        and full_diag.get("rhd_core_changes_required")==[]
        and full_diag.get("lex0_entries_generated")==0
        and full_diag.get("human_validation_claimed") is False
        and credit.get("completes_second_source_end_to_end_gate") is True
    )

    manifest={
      "manifest_id":"RHD-STEFFEL-MACHINE-ONLY-RELEASE-MANIFEST-1",
      "generated_from":"repository working tree plus deterministic CI-derived pilot outputs",
      "release_scope":"machine-only historical-digital scholarly edition; zero required human adjudication",
      "human_validation_claimed":False,
      "counts":{
        "canonical_lexical_records":canonical_count,
        ...
        "second_source_pilot_witnesses_checksum_fixed":1 if fixed else 0,
        "second_source_minimal_pilots_complete":1 if minimal_complete else 0,
        "second_source_minimal_canonical_records":minimal_count,
        "second_source_pilots_full_witness_end_to_end_complete":1 if full_complete else 0,
        "second_source_full_witness_canonical_records":full_count,
      },
      "second_source_pilot":{
        ...
        "minimal_end_to_end_completion_credit":bool(minimal_complete),
        "full_witness_end_to_end_completion_credit":bool(full_complete),
        "minimal_canonical_records":minimal_count,
        "full_witness_canonical_records":full_count,
        "full_witness_pdf_pages":full_diag.get("pdf_pages"),
        "full_witness_rhd_core_changes_required":len(full_diag.get("rhd_core_changes_required",[])),
        "full_witness_lex0_entries_generated":full_diag.get("lex0_entries_generated"),
        "minimal_tei_artifact":"data/pilot/tellechea-1826.minimal-pilot.tei.xml",
    ...
    print(f"generated release integrity manifest with {len(records)} hashed artifacts -> {OUT.relative_to(ROOT)}; Tellechea full gate={'closed' if full_complete else 'open'}")
